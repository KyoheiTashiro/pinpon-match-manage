...
ルートフォントサイズは `18px`（`html { font-size: 18px; }`）。以下の rem 値は Tailwind スケール名に対応する。

| 要素         | Tailwindスケール | remサイズ  | 行間 |
| ------------ | ---------------- | ---------- | ---- |
| 本文         | `base`           | `1.125rem` | 1.7  |
| ボタンラベル | `lg`             | `1.375rem` | 1.5  |
| 見出しh2     | `xl`             | `1.75rem`  | 1.3  |
| 見出しh1     | `2xl`            | `2rem`     | 1.2  |
| スコア表示   | `score`          | `2.5rem`   | 1.0  |
| 小テキスト   | `sm`             | `1rem`     | 1.6  |
| 極小テキスト | `xs`             | `0.875rem` | 1.4  |
...
| 成功（勝ち）             | `#16A34A` (green-600) | 5.6:1 → 太字+枠で補強                                                                                                                                        |
| 危険（削除・リセット）   | `#B91C1C` (red-700)   | 6.6:1                                                                                                                                                        |
| 警告                     | `#FBBF24` (amber-400) | solid時 暗文字`ink`を載せる（背景としては高コントラスト）。soft時 amber文字は対白2.3:1で低いが、ラベル文字列で冗長化し色のみに依存しない補助バッジ用途に限定 |
| 背景                     | `#FFFFFF`             |                                                                                                                                                              |
| サブ背景                 | `#F1F5F9` (slate-100) |                                                                                                                                                              |
| 罫線                     | `#475569` (slate-600) | 7.5:1                                                                                                                                                        |
| 勝ち背景                 | `#DCFCE7` (green-100) |                                                                                                                                                              |
| 負け背景                 | `#FEE2E2` (red-100)   |                                                                                                                                                              |
...
- スクリーンリーダー読上対応（対戦一覧は `<th scope>` 適切設定）
