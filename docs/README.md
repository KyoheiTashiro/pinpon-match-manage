...
- アクセシビリティ: キーボード入力可・WCAG 2.1 **AA準拠目標**（本文・主要操作はコントラスト比4.5:1以上、大きな文字・UI部品は3:1以上）。状態バッジ等の補助情報はラベル文字列で冗長化し、色のみに依存しないことで担保（色のコントラスト比は問わない）
- PWA: ホーム画面追加可・初回ロード後オフライン全機能動作・更新時 自動再取得
- 視認性: 高齢者前提。本文最低18px・主要操作ボタン最低56px高・本文/主要操作のコントラスト比4.5:1以上
