...
**配置先 = 「結果」タブ（`/result` = `ResultTab`, `src/features/tournament/ranking/index.tsx`）。**
スコアボードの結果ビュー（`MatchResultView`）には**置かない**。

- 結果タブの構成: 順位表 → **対戦結果**（試合ごとの G1〜G5 スコア表）。
- この **対戦結果セクションに、試合ごとの点数進行グラフを追加**する。
  - 各試合 = 「対戦者名 + その試合の各ゲームのグラフ（ゲームごと2段）」のまとまり。
  - 既存の対戦結果テーブルは残し、その下（または各試合カード内）にグラフを置く。
- **画像保存対象に含める**: 結果タブは `useImageCapture('結果')` の `ref` 内を画像化する。
  グラフも `ref` 内に置き、保存画像に含める（白背景前提が効く理由）。
- **swap概念なし**: 結果タブは常に `leftName`/`rightName` 固定表示（上段=左・下段=右）。
- `pointLog` を持つゲームのみ描画。持たない試合（旧データ）はテーブルのみでグラフ省略。
...
- **第2段階**: グラフUI描画（SVG）を **結果タブ（`/result`）の対戦結果**に配置。白背景・画像保存対応。
